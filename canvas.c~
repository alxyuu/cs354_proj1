...
double cone_height = 1.0;
double cone_radius = 1.0;
int    cone_tri    = 8;


GLfloat inside   = 1.0f;
GLfloat outside  = 1.5f;
...
void myDisplay (void) {
	...
	switch (disp_mode) {
		...
		case DM_CONE_TRI_CALC:
			 ...
			 */
			draw_cone_tri_calc(cone_height, cone_radius, cone_tri);
			break;
		case DM_VRML:
			draw_vrml();
			break;
...
void myKeyHandler(unsigned char ch, int x, int y) {
	switch(ch) {
		...
		case '7':
			if(outside > inside + 0.1f && outside > 0.2f)
				outside -= 0.1f;
			break;
		case '9':
			outside += 0.1f;
